...
**此组件在iview-area组件基础上做的扩展，增加了clearable功能**，只对al-selector做了修改
...
# install 安装
```
    npm install iview-area-bin --save
```
# use 使用
在main.js中写入下面的代码
```javascript
    import iviewArea from 'iview-area-bin';
    import Vue from 'vue';
    Vue.use(iviewArea);
```
...
clearable|是否有清空提示|Boolean|false
